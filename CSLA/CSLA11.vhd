...
-- ================================================================
-- CSLA11 (Carry Select Adder de 11 bits)
--
-- Objetivo:
...
entity CSLA11 is
    Port (
        ...
        A     : in  STD_LOGIC_VECTOR(10 downto 0); -- Operando de 11 bits
        B     : in  STD_LOGIC_VECTOR(10 downto 0); -- Operando de 11 bits
        S     : out STD_LOGIC_VECTOR(10 downto 0); -- Resultado da soma
        Cout  : out STD_LOGIC                      -- Carry final (vai-um do bit mais significativo)
    );
end CSLA11;

architecture Behavioral of CSLA11 is
    -- S0(i): soma do bit i assumindo carry_in = 0
    -- S1(i): soma do bit i assumindo carry_in = 1
    signal S0, S1: STD_LOGIC_VECTOR(10 downto 0);

    -- Vetor de carries reais encadeados entre os bits.
    -- Atenção ao índice -1:
    --   C(-1) representa o carry "antes" do bit 0 (recebe Cin).
    --   C(0) é carry de saída do bit 0, ..., C(10) do bit 10.
    ...
    signal C0, C1: STD_LOGIC_VECTOR(10 downto 0);
    
begin
    -- Define o carry inicial da cadeia.
    C(-1) <= Cin;

    -- Gera 11 estágios idênticos (um para cada bit, de 0 a 10).
    gen_csa: for i in 0 to 10 generate
    ...
    -- Carry final do somador de 11 bits.
    Cout <= C(10);
end Behavioral;
