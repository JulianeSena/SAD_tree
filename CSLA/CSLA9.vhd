...
-- ================================================================
-- CSLA9 (Carry Select Adder de 9 bits)
--
-- Objetivo:
...
entity CSLA9 is
    Port (
        ...
        A     : in  STD_LOGIC_VECTOR(8 downto 0); -- Operando de 9 bits
        B     : in  STD_LOGIC_VECTOR(8 downto 0); -- Operando de 9 bits
        S     : out STD_LOGIC_VECTOR(8 downto 0); -- Resultado da soma
        Cout  : out STD_LOGIC                     -- Carry final (vai-um do bit mais significativo)
    );
end CSLA9;

architecture Behavioral of CSLA9 is
    ...
    --   C(0) é carry de saída do bit 0, ..., C(8) do bit 8.
    ...
    signal C0, C1: STD_LOGIC_VECTOR(8 downto 0);
    
begin
    -- Define o carry inicial da cadeia.
    C(-1) <= Cin;

    -- Gera 9 estágios idênticos (um para cada bit, de 0 a 8).
    gen_csa: for i in 0 to 8 generate
    ...
    -- Carry final do somador de 9 bits.
    Cout <= C(8);
end Behavioral;
